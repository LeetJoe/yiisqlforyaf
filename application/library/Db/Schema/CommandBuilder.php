<?php
/**
 * Db_Schema_CommandBuilder class file.
 *
...
/**
 * Db_Schema_CommandBuilder provides basic methods to create query commands for tables.
 *
 * @property Db_Connection    $dbConnection Database connection.
 * @property Db_Schema_Schema $schema The schema for this command builder.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.schema
 * @since 1.0
 */
class Db_Schema_CommandBuilder
{
	...
	/**
	 * @param Db_Schema_Schema $schema the schema for this command builder
	 */
	...
	/**
	 * @return Db_Schema_Schema the schema for this command builder.
	 */
	 ...
	 * @param mixed $table the table schema ({@link Db_Schema_TableSchema}) or the table name (string).
	 ...
	 *
*@param mixed                    $table the table schema ({@link Db_Schema_TableSchema}) or the table name (string).
	 * @param Db_Schema_Criteria $criteria the query criteria
	 * @param string             $alias the alias name of the primary table. Defaults to 't'.
	 *
*@return Db_Command query command.
	 */
...
*@param mixed                    $table the table schema ({@link Db_Schema_TableSchema}) or the table name (string).
	 * @param Db_Schema_Criteria $criteria the query criteria
	 *
	 * @return Db_Command delete command.
	 */
...
*@param mixed $table the table schema ({@link Db_Schema_TableSchema}) or the table name (string).
	 ...
	 *
	 * @return Db_Command insert command
	 */
	 ...
	 *
*@param Db_Schema_TableSchema    $table the table schema ({@link CDbTableSchema}) or the table name (string).
	 * @param array              $data list of columns to be updated (name=>value)
	 * @param Db_Schema_Criteria $criteria the query criteria
	 *
	 * @return Db_Command update command.
	 */
...
*@param mixed                    $table the table schema ({@link Db_Schema_TableSchema}) or the table name (string).
	 ...
	 * @param Db_Schema_Criteria $criteria the query criteria
	 *
	 *@return Db_Command the created command
	...
	public function createUpdateCounterCommand($table,$counters,$criteria)
	{
		...
		if($fields!==array())
		{
			...
			throw new Exception('No counter columns are being updated for table "{table}".');
	}
...
*@return Db_Command the created command
	 ...
	 *
*@param Db_Command $command database command
...
*@param mixed $condition query condition or criteria.
	 ...
	 *
*@return Db_Schema_Criteria the created query criteria
	 * @throws CException if the condition is not string, array and Db_Schema_Criteria
	 */
	public function createCriteria($condition='',$params=array())
	{
		if(is_array($condition))
			$criteria=new Db_Schema_Criteria($condition);
		elseif($condition instanceof Db_Schema_Criteria)
			$criteria=clone $condition;
		else
		{
			$criteria=new Db_Schema_Criteria;
	...
	public function createColumnCriteria($table,$columns,$condition='',$params=array(),$prefix=null)
	{
		...
		foreach($columns as $name=>$value)
		{
			if(($column=$table->getColumn($name))!==null)
			{
				...
				throw new Exception('Table "{table}" does not have a column named "{column}".');
		}
...
*@return string the expression for selection
			...
			foreach($columnName as $name)
			{
				...
				for($i=0;$i<$n;++$i)
				{
					if(isset($values[$i][$name]))
					{
						...
						throw new Exception('The value for the column "{column}" is not supplied when querying the table "{table}".');
				}
			}
			if(count($values)===1)
			{
	 ...
	 *
*@param Db_Schema_TableSchema $table the table schema
	 * @param array           $values list of primary key values to be selected within
...
*@return string the expression for selection
	 ...
	 * @param mixed $table table schema ({@link Db_Schema_TableSchema}) or table name (string).
