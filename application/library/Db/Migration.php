...
/**
 * Db_Migration is the base class for representing a database migration.
 *
 * Db_Migration is designed to be used together with the "yiic migrate" command.
 *
 * Each child class of Db_Migration represents an individual database migration which
 ...
 *
 * Db_Migration provides a set of convenient methods for manipulating database data and schema.
 ...
 * Compared with the same methods in {@link Db_Command}, these methods will display extra
 * information showing the method parameters and execution time, which may be useful when
 * applying migrations.
 *
 * @property Db_Connection $dbConnection The currently active database connection.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db
 * @since 1.1.6
 */
abstract class Db_Migration
{
	private $_db;
	 ...
	 * @return Db_Connection the currently active database connection
	 */
	public function getDbConnection()
	{
		if($this->_db===null)
		{
			...
			if(!$this->_db instanceof Db_Connection)
				throw new Exception('The "db" application component must be configured to be a Db_Connection object.');
		}
		return $this->_db;
	}
	 ...
	 *
*@param Db_Connection $db the database connection component
	 */
	 ...
	 * @param array $params input parameters (name=>value) for the SQL execution. See {@link Db_Command::execute} for more details.
	 * @since 1.1.7
	 */
	 ...
	 * refer to {@link Db_Command::where} on how to specify conditions.
	...
	{
		echo "    > alter table $table drop constraint $name primary key xxx ...";
		$time=microtime(true);
