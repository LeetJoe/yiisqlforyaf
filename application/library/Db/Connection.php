...
/**
 * Db_Connection represents a connection to a database.
 *
 * Db_Connection works together with {@link Db_Command}, {@link Db_DataReader}
 ...
 *
 * The following example shows how to create a Db_Connection instance and establish
 * the actual connection:
 * <pre>
 * $connection=new Db_Connection($dsn,$username,$password);
 ...
 * </pre>
 *
 * Db_Connection also provides a set of methods to support setting and querying
 * of certain DBMS attributes, such as {@link getNullConversion nullConversion}.
 *
 * Since Db_Connection implements the interface IApplicationComponent, it can
 ...
 * @property Db_Transaction           $currentTransaction The currently active transaction. Null if no active transaction.
 * @property Db_Schema_Schema         $schema The database schema for the current connection.
 * @property Db_Schema_CommandBuilder $commandBuilder The command builder.
...
class Db_Connection
{
	 ...
	 * effective when the Db_Connection object is used as an application component.
	 */
	 ...
	 * By setting this property, any token like '{{tableName}}' in {@link Db_Command::text} will
	...
	public $driverMap=array(
		'pgsql'=>'PgsqlSchemaSchema',    // PostgreSQL
		'mysqli'=>'Db_Schema_Mysql_Schema',   // MySQL
		'mysql'=>'Db_Schema_Mysql_Schema',    // MySQL
		'sqlite'=>'SqliteSchemaSchema',  // sqlite 3
		'sqlite2'=>'SqliteSchemaSchema', // sqlite 2
		'mssql'=>'MssqlSchemaSchema',    // Mssql driver on windows hosts
		'dblib'=>'MssqlSchemaSchema',    // dblib drivers on linux (and maybe others os) hosts
		'sqlsrv'=>'MssqlSchemaSchema',   // Mssql
		'oci'=>'OciSchemaSchema',        // Oracle driver
	);
	...
	public static function instance($db = 'db') {
		if (!isset(self::$dbArray[$db]) || empty(self::$dbArray[$db])) {
			$config = Config::ini('application.ini', 'mysql');
			if (isset($config[$db])) {
				self::$dbArray[$db] = new Db_Connection($config[$db]['connectionString'], $config[$db]['username'], $config[$db]['password']);
			}
		}
		return self::$dbArray[$db];
	}
	 ...
	 * when the Db_Connection is used as an application component.
...
*@param integer $duration the number of seconds that query results may remain valid in cache.
	 ...
	 *
*@return Db_Connection the connection instance itself.
	 * @since 1.1.7
	 */
	...
	protected function open()
	{
		if($this->_pdo===null)
		{
			if(empty($this->connectionString))
				throw new Exception('Db_Connection.connectionString cannot be empty.');
			try
			{
				# Yii::trace('Opening DB connection','system.db.Db_Connection');
			...
			{
				throw new Exception('Db_Connection failed to open the DB connection.');
			}
		}
	}
...
*@param mixed $query the DB query to be executed. This can be either a string representing a SQL statement,
	 ...
	 *
*@return Db_Command the DB command
	 */
	public function createCommand($query=null)
	{
		$this->setActive(true);
		return new Db_Command($this,$query);
	}

	/**
	 * Returns the currently active transaction.
	 * @return Db_Transaction the currently active transaction. Null if no active transaction.
	 */
	...
	{
		# Yii::trace('Starting transaction','system.db.Db_Connection');
		$this->setActive(true);
		$this->_pdo->beginTransaction();
		return $this->_transaction=new Db_Transaction($this);
	}

	/**
	 * Returns the database schema for the current connection
	 * @return Db_Schema_Schema the database schema for the current connection
	 */
	public function getSchema()
	{
		...
		else
		{
			...
			if(isset($this->driverMap[$driver]))
            {
                ...
                $class=new ReflectionClass($this->driverMap[$driver]);
                return call_user_func_array(array($class,'newInstance'),array($this));
            }
			else
				throw new Exception('Db_Connection does not support reading schema for {driver} database.');
		}
	}

	/**
	 * Returns the SQL command builder for the current DB connection.
	 * @return Db_Schema_CommandBuilder the command builder
	 */
