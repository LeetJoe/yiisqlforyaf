<?php
/**
 * This file contains the Db_Command class.
 *
...
/**
 * Db_Command represents an SQL statement to execute against a database.
 *
 ...
 * can be accessed via the returned {@link Db_DataReader}.
 *
 * Db_Command supports SQL statment preparation and parameter binding.
 ...
 *
 * Starting from version 1.1.6, Db_Command can also be used as a query builder
 ...
 * @property Db_Connection $connection The connection associated with this command.
...
class Db_Command
{
	 ...
	 * Constructor.
	 *
*@param Db_Connection $connection the database connection
	 ...
	 * Since 1.1.7 it is possible to use a specific mode of data fetching by setting
 	 ...
 	 * for more details.
	 */
	public function __construct(Db_Connection $connection, $query=null)
	{
...
*@return Db_Command
	 ...
	 * @return Db_Command this command instance
	 * @since 1.1.6
	 */
...
*@return Db_Command this command instance
	...
	/**
	 * @return Db_Connection the connection associated with this command
	 */
	...
	public function prepare()
	{
		if($this->_statement==null)
		{
			...
			{
				# Yii::log('Error in preparing SQL: '.$this->getText(),CLogger::LEVEL_ERROR,'system.db.Db_Command');
				$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
				throw new Exception(Yii::t('yii','Db_Command failed to prepare the SQL statement: {error}',
...
*@param mixed $name Parameter identifier. For a prepared statement
	 ...
	 *
	 * @return Db_Command the current command being executed
...
*@param array $values the values to be bound. This must be given in terms of an associative
	...
	public function execute($params=array())
	{
		if($this->_connection->enableParamLogging && ($pars=array_merge($this->_paramLog,$params))!==array())
		{
			...
			$par='';
		//Yii::trace('Executing SQL: '.$this->getText().$par,'system.db.Db_Command');
		try
		{
			//if($this->_connection->enableProfiling)
				//Yii::beginProfile('system.db.Db_Command.execute('.$this->getText().$par.')','system.db.Db_Command.execute');
			...
			//if($this->_connection->enableProfiling)
				//Yii::endProfile('system.db.Db_Command.execute('.$this->getText().$par.')','system.db.Db_Command.execute');
		...
		catch(Exception $e)
		{
			...
			//Yii::log(Yii::t('yii','Db_Command::execute() failed: {error}. The SQL statement executed was: {sql}.',
				//array('{error}'=>$message, '{sql}'=>$this->getText().$par)),CLogger::LEVEL_ERROR,'system.db.Db_Command');

			//if(YII_DEBUG)
				//$message.='. The SQL statement executed was: '.$this->getText().$par;

			throw new Exception(Yii::t('yii','Db_Command failed to execute the SQL statement: {error}',
...
*@param array $params input parameters (name=>value) for the SQL execution. This is an alternative
	 ...
	 * their real data types, you have to use {@link bindParam} or {@link bindValue} instead.
	 *
*@return Db_DataReader the reader object for fetching the query result
	 * @throws CException execution failed
	 */
	...
	private function queryInternal($method,$mode,$params=array())
	{
		...
		//Yii::trace('Querying SQL: '.$this->getText().$par,'system.db.Db_Command');
				...
				&& ($cache=Yii::app()->getComponent($this->_connection->queryCacheID))!==null)
		{
			...
			{
				Yii::trace('Query result found in cache','system.db.Db_Command');
				return $result[0];
			}
		}
		*/
		...
		{
			//if($this->_connection->enableProfiling)
				//Yii::beginProfile('system.db.Db_Command.query('.$this->getText().$par.')','system.db.Db_Command.query');
			...
			//if($this->_connection->enableProfiling)
				//Yii::endProfile('system.db.Db_Command.query('.$this->getText().$par.')','system.db.Db_Command.query');
		...
		catch(Exception $e)
		{
			...
			$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
			$message=$e->getMessage();
			//Yii::log(Yii::t('yii','Db_Command::{method}() failed: {error}. The SQL statement executed was: {sql}.',
				//array('{method}'=>$method, '{error}'=>$message, '{sql}'=>$this->getText().$par)),CLogger::LEVEL_ERROR,'system.db.Db_Command');
...
*@param mixed $columns the columns to be selected. Defaults to '*', meaning all columns.
	 ...
	 *
	 *@return Db_Command the command object itself
	 * @since 1.1.6
	 */
