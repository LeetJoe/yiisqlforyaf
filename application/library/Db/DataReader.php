...
/**
 * Db_DataReader represents a forward-only stream of rows from a query result set.
 *
 * To read the current row of data, call {@link read}. The method {@link readAll}
 * returns all the rows in a single array.
 *
 * One can also retrieve the rows of data in Db_DataReader by using foreach:
 * <pre>
 * foreach($reader as $row)
 *     // $row represents a row of data
 * </pre>
 * Since Db_DataReader is a forward-only stream, you can only traverse it once.
 *
 ...
 * @since 1.0
 */
class Db_DataReader implements Iterator, Countable
{
	private $_statement;
	 ...
	 * Constructor.
	 *
*@param Db_Command $command the command generating the query result
	 */
	public function __construct(Db_Command $command)
	{
	...
	public function rewind()
	{
		if($this->_index<0)
		{
			...
			throw new Exception('Db_DataReader cannot rewind. It is a forward-only reader.');
	}
